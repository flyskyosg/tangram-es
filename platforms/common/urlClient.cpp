...
void UrlClient::cancelRequest(UrlRequestHandle handle) {
    ...
    for (auto it = m_requests.begin(), end = m_requests.end(); it != end; ++it) {
        ...
        if (request.handle == handle) {
            ...
            // FIXME: The recent refactor of NetworkDataSource has created a deadlock when
            // a tile request is canceled and runs its callback before the cancel call
            // returns. Marking the request as canceled here causes a curl loop to perform
            // the callback later, but this involves more total work to cancel the request.
