...
void AndroidPlatform::setupJniEnv(JNIEnv* jniEnv) {
    ...
    startUrlRequestMID = jniEnv->GetMethodID(tangramClass, "startUrlRequest", "(Ljava/lang/String;J)Lokhttp3/Call;");
    cancelUrlRequestMID = jniEnv->GetMethodID(tangramClass, "cancelUrlRequest", "(Lokhttp3/Call;)V");
...
UrlRequestHandle AndroidPlatform::startUrlRequest(Url _url, UrlCallback _callback) {
    ...
    // FIXME: We shouldn't handle file requests synchronously.
    ...
    // Call the MapController method to start the URL request, returning a reference to an Okhttp3.Call object.
    jobject callObject = jniEnv->CallObjectMethod(m_tangramInstance, startUrlRequestMID, jUrl, jCallbackPtr);

    // Create a weak global reference to the Call. This is just a Java object pointer, but the referenced
    // object is allowed to be GC'ed while this weak reference is held. We can pass this reference back
    // to a Java method and if the object is gone, the reference will equal 'null'.
    // http://docs.oracle.com/javase/7/docs/technotes/guides/jni/spec/functions.html#weak
    // FIXME: I think we leak these when the call isn't canceled, we should call DeleteGlobalRef too.
    jweak callReference = jniEnv->NewWeakGlobalRef(callObject);

    // Check that it's safe to convert Java object references to integers and back:
    assert(callReference == reinterpret_cast<jweak>(reinterpret_cast<UrlRequestHandle>(callReference)));

    // Reinterpret the weak reference as an integer handle and return it. This lets us directly access
    // the Call object later if it still exists, and not worry about leaking its memory.
    return reinterpret_cast<UrlRequestHandle>(callReference);
}
...
void AndroidPlatform::cancelUrlRequest(UrlRequestHandle request) {
    ...
    // Our UrlRequestHandle was created by reinterpreting a weak global reference as an integer, so
    // we can reinterpret it back to a weak reference and pass it into the MapController cancel method.
    jweak callReference = reinterpret_cast<jweak>(request);

    jniEnv->CallVoidMethod(m_tangramInstance, cancelUrlRequestMID, callReference);
}

void AndroidPlatform::onUrlSuccess(JNIEnv* _jniEnv, jlong _jCallbackPtr, jbyteArray _jBytes) {
    UrlResponse response;
...
void AndroidPlatform::onUrlFailure(JNIEnv* _jniEnv, jlong _jCallbackPtr, jstring _jError) {
    UrlResponse response;
